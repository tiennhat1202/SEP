import React, { useEffect, useState } from 'react';
import EditBtn from '../Commons/EditButton'
import DeleteBtn from '../Commons/DeleteBtn'
import { useParams } from 'react-router-dom'
import MealService from '../../services/MealService'
import { toast } from 'react-toastify';
import Header from '../DefaultLayout/Header_admin/Nav'
import SideBar from '../DefaultLayout/Sidebar/SideBar'
function TableMealListDetails() {
  const [product, setProduct] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const { id } = useParams();
  // Fetching Data
  useEffect(() => {
    const mealService = new MealService();
    /* useEffect(() => {
      console.log(product);
    }, [product]); */
    const getMealsById = async () => {
      try {
        const res = await mealService.getMealsById(id);
        if (res) {
          setProduct(res);
          setIsLoading(false);
        }
      } catch (error) {
        toast.error('Error fetching meals:', error);
        setIsLoading(false);
      }
    };
    if (isLoading) {
      getMealsById();
    }
  }, [id, isLoading]);
  if (isLoading) {
    return <div>Loading...</div>;
  }
  return (
    <div>
      <Header></Header>
      <SideBar></SideBar>
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              <th scope="col" className="px-6 py-3">
                <span className="sr-only">Image</span>
              </th>
              <th scope="col" className="px-6 py-3">
                Product name
              </th>
              <th scope="col" className="px-6 py-3">
                <div className="flex items-center">
                  Category
                  <a href="#">
                    <svg
                      className="w-3 h-3 ml-1.5"
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z" />
                    </svg>
                  </a>
                </div>
              </th>
              <th scope="col" className="px-6 py-3">
                <div className="flex items-center">
                  Price
                  <a href="#">
                    <svg
                      className="w-3 h-3 ml-1.5"
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z" />
                    </svg>
                  </a>
                </div>
              </th>
              <th scope="col" className="px-6 py-3">
                <div className="text-center">Action</div>
              </th>
            </tr>
          </thead>
          <tbody>
            {product?.map((item) => (
              <tr
                key={item.key}
                className="bg-white border-b dark:bg-gray-800 dark:border-gray-700"
              >
                <td className="w-32 p-6">
                  <img
                    className="w-20 ml-4"
                    src="{item.img}"
                    alt="{item.name}"
                  ></img>
                </td>
                <th
                  scope="row"
                  className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"
                >
                  {item.name}
                </th>
                <td className="px-6 py-4"></td>
                {/* <td className="px-6 py-4">$2999</td> */}
                <td>
                  <div className="flex justify-center gap-3">
                    <EditBtn edit_id={`/admin/menu_management/update_menu/${item.id}`}></EditBtn>
                    <a href="#">
                      <DeleteBtn></DeleteBtn>
                    </a>
                  </div>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <div className="mt-3 text-right"></div>
    </div>
  );
}
export default TableMealListDetails